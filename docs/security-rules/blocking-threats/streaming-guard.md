...
| `min_risk`                  | number (0-1), optional           | not set                     | Confidence floor: an Unsafe token whose model probability is below this value counts as Controversial before the per-chunk verdict is formed. Use it to cut low-confidence false alarms on borderline text; requires `severity_threshold: "Unsafe"`. Unset = no floor. |
| `sustained_unsafe_min_run`  | integer (2-64), optional         | not set                     | Require this many consecutive Unsafe tokens before the guard enforces; an isolated Unsafe token counts as Controversial. Cuts single-token false alarms while a genuine run of unsafe output still blocks. Accepted range 2-64. Requires `severity_threshold: "Unsafe"`. Unset = one Unsafe token enforces. |
| `guard_system_preamble`     | string (1-2000), optional        | not set                     | Situational context the guard is primed with (its first system message), e.g. "this is the bank's in-app assistant; the user is the authenticated account owner". Tells the guard what the assistant is so it stops flagging the user's own data as a violation. Only set it when your data source scopes every query to the user's own accounts — otherwise it can let a different person's data through. 1-2000 characters; an empty string is refused. Unset = no preamble. |
